...
public class DoubleFFT extends FFT1D<double[]> implements RealFFT<double[]> {
	...
	@Override
	protected void bitReverse(final double[] data, int from, int to) {	
		// Change from complex indices to double[] indices.
		from <<= 1;
		to <<= 1;
		
		// The number of bits flipped is +1 on either side of the address sequence
		// due to the double[] packing...
	 ...
	 */
	@Override
	protected void merge2(final double[] data, int from, int to, final boolean isForward, final int blkbit) {	
	 ...
	 */
	@Override
	protected void merge4(final double[] data, int from, int to, final boolean isForward, final int blkbit) {	
		// The double[] block size
		final int blk = 1 << (blkbit+1);
	...
	void realTransform(final double data[], final int addressBits, final boolean isForward) {
		...
		final int chunks = Math.min(getChunks(), 1<<(addressBits-1));
	 ...
	 */
	@Override
	public final void realTransform(final double[] data, final boolean isForward, int chunks) {	
	...
	void realTransform(final double[] data, final int addressBits, final boolean isForward, int chunks) {	
		...
		final double dn = (double) (2<<addressBits) / chunks;
	 ...
	 */
	private void scale(final double[] data, final int length, final double value, int threads) {
		if(threads < 2) {
			...
			final Queue queue = new Queue();
			final double dn = (double) length / threads;
	 ...
	 */
	@Override
	public int countOps(double[] data) {
